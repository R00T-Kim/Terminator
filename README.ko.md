...
| **chain** | 다단계 익스플로잇: leak -> overwrite -> shell | Opus | Max | `solve.py` |
| **critic** | Security Council 심의 (5 아키타입) + 교차 검증 | Opus | High | `critic_review.md` |
| **verifier** | 로컬 3회 재현 -> 원격 실행 | Sonnet | Low | `FLAG_FOUND` |
...
| **analyst** | CVE 매칭, source->sink 추적, 신뢰도 점수화 | Sonnet | High | `vulnerability_candidates.md` |
| **exploiter** | PoC 개발, 품질 등급 분류 (Evidence Tier E1-E4) | Opus | Max | PoC 스크립트 + 증거 |
...
| **patch-hunter** | 보안 커밋에서 불완전 수정 및 변종 취약점 탐색 | Sonnet | High | `patch_analysis.md` |
...
| **observation_mask_hook.py** | PostToolUse (Bash/Read) | 500줄 초과 출력을 파일로 자동 저장, 컨텍스트 오버플로우 방지 |
...
│   │   ├── observation_mask_hook.py # 대용량 출력 마스킹 (PostToolUse)
